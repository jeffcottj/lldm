...
**Status:** Phases 0 and 1 complete; Phase 2 planned and ready for implementation
...
The repository at `/home/john/repos/lldm` is initialized on `main`, tracks its GitHub `origin`, and contains the completed [Phase 0 executable foundation](docs/plans/PHASE_0.md): a locked Node 24/pnpm workspace, strict quality tooling and CI, the private `@lldm/contracts`, `@lldm/engine`, and `@lldm/content` packages, runtime schemas, pure resolution rules, 87 passing baseline tests, generated mechanical references, and syntax-validated Compose and Wrangler placeholders. [Phase 1](docs/plans/PHASE_1.md) is complete: buildable private `@lldm/runtime` and `@lldm/cli` units establish the fixed dependency boundaries; Kysely and `better-sqlite3` are isolated behind the runtime's package-internal SQLite seam; and Accepted ADR-0002 records the deterministic runtime architecture. Central contract registries own the command, event, content, projection, proposal, and transport unions, with explicit command/event revisions, distinct storage/state/randomness/content version concepts, typed transaction outcomes, opaque Phase 1 identities, versioned random evidence, validated snapshot records, compensating-undo records, and public/private/host projection schemas. Contracts define separate playable state, rank/resource structure, named-zone combat and physical continuation, Progress/Danger, social, ritual, inert mechanical-effect, content-catalog, starter-loadout, and immutable-manifest records with semantic validators. The approved Phase 1 rules are locked in executable constants and generated documentation. A pure versioned campaign state, exhaustive event applier, and invariant validator exist. Check decisions use explicit recorded HMAC-SHA-256 draws for simulated outcomes and a nonce-bound, two-transaction pre-disclosed path for physical rolls and Spark conversion. Character decisions materialize campaign-pinned rank-one state, bind ready/spent significant-gear mechanics, enforce exact resources and recovery, expose legal abilities, and support prerequisite-checked structural advancement through rank 4. Combat decisions enforce named-zone range, hero-first alternating activations, legal actions, deterministic enemy fallback, reactions, fixed Impact and armor, Wounds, boss transitions/objectives, and physical death continuation. Challenge, social, and ritual decisions share the authoritative check flow, enforce hard limits and declared prerequisites before draws, apply pinned outcome facts through typed events, and replay through strengthened event invariants. The production catalog pins 54 canonical revision-one definitions, six mechanically distinct archetypes, four starter loadouts, the Floodgate encounter, and its challenge/social/ritual slice under manifest hash `sha256:7663c17e9a9cb83b5ec88e096c32fe735cafdabd2bf55c1127b6b288d9ab735b`; generated playable-content documentation derives from it. A runtime-validated deterministic fixture and a fresh-database CLI runner drive that slice through recoverable physical continuations and valid combat/challenge/social/ritual conclusions; bounded fixed-seed sanity observations add no win-rate gate. The reusable runtime has SQLite-independent typed ports and an atomic command coordinator that permanently binds command/transaction identities, short-circuits exact retries before side effects, commits accepted or canonical rejected event ranges, records deterministic random evidence, and hashes every mechanical transition. Checksummed SQLite migration 1 provides strict local campaign, command, transaction, event, snapshot, and projection tables, verified pre-migration sibling backups, explicit schema-state detection, WAL/foreign-key/full-synchronous connection policy, real immediate atomic commits, canonical revalidation on read, and exact retry across reopen. Verified event-only replay checks every transaction boundary and cached head; atomically written scene/session/threshold snapshots accelerate replay, while invalid snapshots report a structured fallback to full replay. A read-only audit re-executes stored commands against pinned content without changing history. Pure engine-enumerator-backed public-TV, character/eligible-seat-private, and host-control projectors replace derived rows atomically at every stream revision; public views filter private facts, and explicit replay rebuilds compare byte-for-byte without changing canonical rows. Compensating undo exhaustively classifies event families, appends linked inverse transactions for safe latest changes, invalidates unresolved physical nonces while restoring their costs, and rejects prior undo, submitted dice, permanent death, stale targets, and non-invertible dependencies. The scriptable CLI exposes explicit migration, campaign, command, scenario, replay/audit, snapshot, projection, and undo operations with versioned JSON and no application/network/provider behavior. A fixed-seed fresh-database run records 172 transactions, 437 events, and final hash `sha256:74d159fb4d1b2682078010d017cfae867f06f61723270ff7c3eeae512a560558`; cross-process tests prove retry, collision, stale rejection, physical restart/one-use submission, undo, snapshot verification, and byte-identical projection rebuild. The Phase 1 exit audit passes the frozen install, root verification with 217 tests in 32 files, dependency and purity checks, repeated generated-reference drift checks, and explicit deferred-scope checks under Node 24.19.0 and pnpm 11.13.0. Provider adapters and deployable services do not exist yet; the approved [Phase 2 execution plan](docs/plans/PHASE_2.md) defines their next implementation boundary.

Useful host capabilities already available are Node.js 24, npm, pnpm, Docker Engine and CLI, Docker Buildx and Compose, Podman, Git, GitHub CLI, Playwright CLI, ffmpeg, curl, jq, and OpenSSL. The active user-local baseline is Node 24.19.0 with pnpm 11.13.1, Docker Engine and CLI 29.7.2, Buildx 0.36.1, and Compose 5.4.0; completed Phase 0/1 evidence retains the exact pnpm 11.13.0 version used for those historical audits. Docker is enabled at boot and `john` has non-root daemon access through the `docker` group for Phase 2 appliance work. Rust is not installed and is no longer required because the selected stack is TypeScript throughout. Cloudflare Wrangler is installed as a pinned project development dependency rather than a global tool.
...
Phase 1 is complete. Its delivered scope is:

- Added `packages/runtime` as the reusable impure owner of transaction coordination, injected clock/entropy/ID ports, SQLite/Kysely persistence, migrations, replay, snapshots, projections, backups, and compensating undo. `packages/engine` remains pure and `apps/host` remains deferred to Phase 2.
- Added a thin scriptable `apps/cli` composition layer for migration, campaign, command, scenario, replay, snapshot, projection, and undo workflows without a TUI or room service.
- Extended only the centralized TypeBox command/event/content/projection unions, then implemented pure command decision plus event application, event-level campaign revisions, transaction-level timestamps, pre/post state hashes, and idempotent command identity.
- Derived versioned, domain-separated simulated draws from campaign seed and command identity, recorded each result/reference, returned stored transactions on retry, and resolved physical rolls through separately committed die-submission commands.
- Preserved `CharacterFoundation` as non-playable creation input and added distinct playable state, rank 1–4 structure, Guard/Wounds, Exertion/Spark/Supply, named zones, alternating activations, fixed Impact, death tests, Progress/Danger, social state, and typed ritual kernels.
- Shipped six playable rank-one archetypes and only the supporting options/enemies/examples needed for a four-hero vertical slice; paths, rank-3 talents, rank-4 capstones, broad catalogs, and balance targets remain later work.
- Extended the generated mechanical reference, retained the probability report, added a generated playable-content reference, and kept this primary plan synchronized through the exit audit.

The Phase 1 exit audit passes 217 tests in 32 files, including the fixed-seed fresh-database and cross-process recovery scenarios. Phase 2 is the next implementation phase. Its detailed execution plan is approved, but no Phase 2 application, provider, relay, PWA, or appliance code exists yet.
...
**Detailed execution plan:** [`docs/plans/PHASE_2.md`](docs/plans/PHASE_2.md)

Phase 2 is planned and not yet implemented. Its locked scope is:

- Build the Fastify appliance host as a composition layer over `packages/runtime`, plus a browser-first React/Vite role-based PWA, filtered snapshot/delta delivery, QR joining, approval, explicit host transfer/recovery, typed guided commands, die entry, correction, reconnect, and explicit session resume.
- Separate untrusted room-scoped `ClientCommand` values from internal `GameCommand` values. Add a replayable local room stream and a crash-recoverable idempotent workflow that links room commands to authoritative mechanical transactions without creating a second mechanical authority.
- Support three to five normal participants with one hero each from six fixed rank-one starters. Add a clearly labeled two-participant rehearsal mode that permits explicit multi-hero assignment and private-view switching.
- Deliver a deterministic 30–45-minute Floodgate guided slice spanning private clues, challenge, social, ritual, tactical combat, one guaranteed pivotal physical roll, optional Spark rolls, controlled branches, and fail-forward conclusions. Use authored three-, four-, and five-hero encounter variants without scaling actor statistics.
- Make the TV primary through concise deterministic narration, public choices, recovery states, and a functional SVG named-zone map. Keep phones glanceable with activation claims, status/reference, select-preview-confirm actions, optional bounded player flavor, private clues, reactions, recent events, and physical die confirmation.
- Deploy the Cloudflare Worker/Durable Object relay with hibernating WebSockets, static PWA assets, token/origin/rate/payload validation, acknowledgements, reconnect routing, and 24-hour cleanup while persisting no campaign or application payload.
- Add fake text provider adapters only, Chromium kiosk startup, and the initial Docker Compose appliance. OpenRouter, general freeform interpretation, voice, generated media, and broad campaign/content systems remain later phases.
- Automate isolated-browser room flows for three, four, and five participants, run a two-phone rehearsal during development, and retain a final deployed-relay signoff with at least three simultaneously connected physical phones.

**Exit criterion:** Automated independent-browser flows for three, four, and five participants and one manual room test with three to five physical phones can join, reconnect, receive correctly filtered state, submit a disclosed physical roll, and finish a supported guided-adventure conclusion without direct local-network access or canonical-state divergence.
...
- Extend the existing physical-roll stakes and confirmation UX with voiced prompts, suspense cues, and interruption-safe audio behavior.
- Add asynchronous scene-card generation, the campaign visual bible, generated battlefield art beneath the existing deterministic combat SVG, asset caching, and audio mixing.
...
- Integration-test the guided room flow for three, four, and five participants, including join approval, private projection filtering, player commands, a physical roll, reconnect, correction, host transfer, and checkpoint recovery with fake providers.
- Keep focused Chromium Playwright paths for the TV and phone interfaces without turning browser-matrix automation into the product. Check real iOS Safari and Android Chrome manually before the Phase 2 room signoff and expand accessibility/device coverage in Phase 6.
...
- Phase 2 is a deterministic guided multi-pillar adventure, not a temporary natural-language parser or early LLM runner.
- Normal room identity separates participant, replaceable connection, and stable character seat; normal participants own one hero, while an explicit two-person rehearsal mode may assign multiple seats.
- Player-host authority is transferable and operational only; it never grants other-seat private clues or runner-internal state.
- Room/session history is a separate local replayable stream linked idempotently to the authoritative mechanical stream; the relay remains ephemeral and payload-free.
- The Phase 2 TV ships a functional deterministic SVG zone map and text narration, while generated art, speech, and audio stay deferred.
