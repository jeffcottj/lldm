...
**Status:** Phases 0 and 1 complete; Phase 2 implemented locally with deployment and physical-device exit evidence pending
...
The repository at `/home/john/repos/lldm` contains the completed Phase 0 foundation and completed [Phase 1 deterministic runtime](docs/plans/PHASE_1.md). Phase 1 retains its immutable campaign manifest hash `sha256:7663c17e9a9cb83b5ec88e096c32fe735cafdabd2bf55c1127b6b288d9ab735b`, migration 1, replay literals, and Accepted ADR-0002 evidence.

The [Phase 2 implementation](docs/plans/PHASE_2.md) now adds private `@lldm/providers`, `apps/host`, `apps/relay`, and `apps/web` workspaces; distinct client/room/game contracts; checksummed SQLite migration 2; a separate hash-checked room stream; filtered combined projections; durable cross-stream workflows; six fixed starters; authored 3/4/5 encounter and reinforcement variants; a deterministic guided Floodgate graph; Fastify TV composition; Cloudflare Worker/Durable Object routing; browser reconnect storage; phone/TV PWA views; and loopback-only Compose/kiosk assets. The Phase 2 mechanical manifest is `sha256:8231d8b34a1e531af298e87c360c7f43e47575a359e04777b6172951656300b7`, the separate presentation manifest is `sha256:802249ccc0aa2b8997bbba7e3ab6ddc57399de8e05ba15cb5b03db74095d689e`, and migration 2 is `sha256:38b882c242fb41685ef4004239d9a3e27211f81573a4a44f5031ddb06beafb27`. Local evidence currently passes 254 tests in 40 files, ten ordered Chromium acceptance scenarios, generated drift, Worker/Compose configuration, final-source Docker image `sha256:b84cca0bcfb405b689b2f9ebec39e2c29cde03d7e0d9d45bf50263aec74ea808`, and test-owned Compose/appliance restart smoke. Phase 2 is not exit-accepted: the household Cloudflare deployment, two-phone rehearsal, and three-to-five-physical-phone signoff remain pending.

Useful host capabilities already available are Node.js 24, npm, pnpm, Docker Engine and CLI, Docker Buildx and Compose, Podman, Git, GitHub CLI, Playwright CLI, ffmpeg, curl, jq, and OpenSSL. The active user-local baseline is Node 24.19.0 with pnpm 11.13.1, Docker Engine and CLI 29.7.2, Buildx 0.36.1, and Compose 5.4.0; completed Phase 0/1 evidence retains the exact pnpm 11.13.0 version used for those historical audits. The long-lived execution shell omitted its configured `docker` supplementary group, so exact Docker evidence ran through `sg docker` under `john`'s existing group membership. Rust is not installed and is no longer required because the selected stack is TypeScript throughout. Cloudflare Wrangler is installed as a pinned project development dependency rather than a global tool.
...
The Phase 1 exit audit passes its preserved 217-test baseline, including the fixed-seed fresh-database and cross-process recovery scenarios. Phase 2 builds on that frozen evidence without changing its manifest or migration literals.
...
Phase 2 application scope is implemented locally. Deployment and physical-device exit acceptance remain pending. Its delivered implementation is:
...
- Include ten ordered Chromium acceptance scenarios spanning isolated TV and 3/4/5-player contexts, full UI terminal paths, test-owned host/relay restarts, workflow crash recovery, rehearsal, claim races, reconnect, no-LAN checks, corrections, reactions, and physical rolls. The two-phone rehearsal and final deployed-relay signoff with at least three simultaneously connected physical phones remain Phase 2 exit work and are not claimed by local automation.
